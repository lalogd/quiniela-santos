...
function selection(element){ 	//Pinta la casilla y actualiza el texto de la quiniela.
    ...
    if (!element.style.backgroundColor){
        ...
        element.style.backgroundColor = "#3375d8f5";
    }
    else{
        if(combinations){
...
function clean(){				//Boton para limpiar la quinela
    ...
    document.getElementById("costo").innerHTML = "Costo: $20";
    document.getElementById("numquinielas").innerHTML = "20 Quiniela(s)";
}
//UPDATEDISPLAY
...
function random(){
    ...
    for (var i = 0; i < 9; i++){
        ...
        partidos[i].getElementsByTagName("span")[r].style.backgroundColor = "#3375d8f5";
        res[i] = ["L","E","V"][r];
    }
    container.innerHTML = res.join("\xa0\xa0");
    costoactual();
...
function send(){ //Envía quiniela al whatsapp
    ...
    window.location.href = "https://wa.me/525521793347?text="+quinbara + whatsapptext;}
}
